...
namespace Section2
  ...
  example : (∀ x, r → p x) ↔ (r → ∀ x, p x) := Iff.intro
    (fun (all_r_to_px : ∀ x, r → p x) (hr : r) (a : α) => show p a from
      have : r → p a := all_r_to_px a
      this hr)
    (fun (r_to_all_px : r → ∀ x, p x) (a : α) (hr : r) => show p a from
      have : ∀ x, p x:= r_to_all_px hr
      this a)
end Section2
...
-- Section 4 --
namespace Section4
  def even (n : Nat) : Prop := ∃k, 2*k = n def odd (n : Nat) : Prop := ¬ even n

  def prime (n : Nat) : Prop := 2 ≤ n ∧ (∀ m, 2 ≤ m → ¬ ∃ k, m*k=n)

  def infinitely_many_primes : Prop := ∀ a, ∃ b > a, prime b

  def Fermat_number (n : Nat) : Prop := ∃ k, 2^2^k + 1 = n
  def Fermat_prime (n : Nat) : Prop := prime n ∧ Fermat_number n

  def infinitely_many_Fermat_primes : Prop := ∀ a, ∃ b > a, Fermat_prime b

  def goldbach_conjecture : Prop := ∀ n > 2, ∃ a b, prime a ∧ prime b ∧ a + b = n

  def Goldbach's_weak_conjecture : Prop := ∀ n > 5, odd n → ∃ a b c, prime a ∧ prime b ∧ prime c ∧ a+b+c = n

  def Fermat's_last_theorem : Prop := ∀ a b c k:Nat, k > 2 → (a^k+b^k ≠ c^k)
end Section4
...
namespace Section5
  ...
  example (a : α) : (∃ x, r → p x) ↔ (r → ∃ x, p x) := Iff.intro
    ...
    (fun h : r → ∃ x, p x => Or.elim (em r)
      (fun hr => have ⟨x, hx⟩ : ∃ x, p x := h hr
        ⟨x, λ _ => hx⟩)
